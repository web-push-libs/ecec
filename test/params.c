...
typedef struct webpush_aesgcm_params_ok_test_s {
  const char* desc;
  ...
  uint32_t rs;
} webpush_aesgcm_params_ok_test_t;

typedef struct webpush_aesgcm_params_err_test_s {
  const char* desc;
  const char* cryptoKey;
  const char* encryption;
  int err;
} webpush_aesgcm_params_err_test_t;

static webpush_aesgcm_params_ok_test_t webpush_aesgcm_params_ok_tests[] = {
  {
...
static webpush_aesgcm_params_err_test_t webpush_aesgcm_params_err_tests[] = {
  {
...
test_webpush_aesgcm_params_ok() {
  ...
  for (size_t i = 0; i < length; i++) {
    webpush_aesgcm_params_ok_test_t t = webpush_aesgcm_params_ok_tests[i];
...
test_webpush_aesgcm_params_err() {
  ...
  for (size_t i = 0; i < length; i++) {
    webpush_aesgcm_params_err_test_t t = webpush_aesgcm_params_err_tests[i];
